...
describe('Performance', () => {
  ...
  describe('.mark()', () => {
    it('should call window.performance.mark with undefined argument', () => {
      ...
      (service as any).mark('doStuff');
      expect(spy).toHaveBeenCalled();
      expect(spy.mock.calls.length).toBe(1);
      expect(spy).toHaveBeenCalledWith('mark_doStuff_undefined');
    });

    it('should call window.performance.mark with the correct argument', () => {
      spy = jest.spyOn(window.performance, 'mark');
      service.mark('doStuff', 'fast');
      expect(spy).toHaveBeenCalled();
      expect(spy.mock.calls.length).toBe(1);
      expect(spy).toHaveBeenCalledWith('mark_doStuff_fast');
    });
  });
  ...
  describe('.measure()', () => {
    it('should call window.performance.measure with the correct arguments', () => {
      ...
      (service as any).measure('doStuff');
      const start = 'mark_doStuff_start';
      const end = 'mark_doStuff_end';
      expect(spy).toHaveBeenCalled();
      expect(spy.mock.calls.length).toBe(1);
      expect(spy).toHaveBeenCalledWith('doStuff', start, end);
    });

    it('should call getDurationByMetric with the correct arguments', () => {
      spy = jest.spyOn(service, 'getDurationByMetric');
      service.measure('doStuff', {});
      expect(spy).toHaveBeenCalled();
      expect(spy.mock.calls.length).toBe(1);
      expect(spy).toHaveBeenCalledWith('doStuff', {});
    });
  });
